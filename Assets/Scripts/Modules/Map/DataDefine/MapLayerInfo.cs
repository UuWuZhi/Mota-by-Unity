...
namespace Modules.Map.DataDefine
{
    ...
    {
        [Header("楼层基础信息")] public int layerId; // 楼层ID（与MapLayerData对应）

        public Tilemap spawnTilemap; // 事件层Tilemap（用于事件系统查找）
        public Transform layerRoot; // 该层的根节点（自身Transform）

        [Header("楼层边界数据")] [Tooltip("楼层边界数据（由编辑器工具自动计算）")]
        public BoundsInt layerBounds; // 楼层网格边界

        [FormerlySerializedAs("_spawnPoints")] [Tooltip("该楼层的所有出生点")] [SerializeField]
        ...
        private void Reset()
        {
            if (spawnPoints.Count == 0) spawnPoints.Add(new SpawnPoint { id = 0, position = transform.position });
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            // 绘制上楼梯出生点（红色方框）
            var upSpawn = spawnPoints.FirstOrDefault(p => p.id == 0);
            if (upSpawn != null)
            {
                Gizmos.color = Color.red;
                // 绘制方框（中心位置为出生点，大小为1x1x0.1的立方体，视觉上为方框）
                Gizmos.DrawWireCube(upSpawn.position + new Vector2(0.5f, 0.5f), new Vector3(1f, 1f, 0.1f));
                Handles.Label(upSpawn.position + Vector2.up * 0.3f, "UpSpawn (ID=0)");
            }

            // 绘制下楼梯出生点（蓝色方框）
            var downSpawn = spawnPoints.FirstOrDefault(p => p.id == 1);
            if (downSpawn == null) return;
            Gizmos.color = Color.blue;
            // 绘制方框（中心位置为出生点，大小为1x1x0.1的立方体，视觉上为方框）
            Gizmos.DrawWireCube(downSpawn.position + new Vector2(0.5f, 0.5f), new Vector3(1f, 1f, 0.1f));
            Handles.Label(downSpawn.position + Vector2.up * 0.3f, "DownSpawn (ID=1)");
        }
#endif
        ...
        {
            var spawnPoint = spawnPoints.Find(p => p.id == id);
            return spawnPoint?.position ?? Vector2.zero;
        }
