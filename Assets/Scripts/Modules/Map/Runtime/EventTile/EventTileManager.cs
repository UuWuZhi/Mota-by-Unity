...
namespace Modules.Map.Runtime.EventTile
{
    public class EventTileManager : MonoBehaviour
    {
        ...
        public void Construct(IGlobalEventVariables globalEventVariables, GridManager gridManager,
            ...
            IEventRunner eventRunner, IEventTileRegistry registry)
        {
        ...
        private void OnEventTileMoved(object sender, TileMovedEventArgs args)
        {
            ...
            try
            {
                ...
                if (!_gridManager || node == null) return;
                try
                {
        ...
        public bool TryTriggerEventTile(Vector3Int cellPos, int? layerId = null)
        {
            ...
            try
            {
                ...
                // 直接执行 tile 上的事件序列
                _eventRunner?.StartSequence(tileMono.sequence, ctx, null);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public void RequestEnterCell_PreMove(Vector3Int cellPos, int layerId, Action<bool, bool> callback,
            Action onExecutionComplete = null)
        {
            //====================================预处理==========================================//
            ...
            try
            {
                ...
                if (executionBlocking)
                {
                    ...
                    {
                        // 阻塞执行事件序列，完成后回调解锁
                        StartCoroutine(_eventRunner.RunSequenceAndWait(tileMono.sequence, ctx, () =>
                        {
                            if (needDecision)
                            {
                                var finalAllow = true;
                ...
                else
                {
                    ...
                    {
                        // 非阻塞执行事件序列
                        _eventRunner?.StartSequence(tileMono.sequence, ctx, () =>
                        {
                            tileMono.EndTrigger();
                            onExecutionComplete?.Invoke();
                        });
