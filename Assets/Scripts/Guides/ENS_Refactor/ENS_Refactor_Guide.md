...
> 【实际处理补充】非 Yarn 对话节点与旧 Flow 节点已提前从工程中删除，以匹配后续阶段的清理方向。
...
*   **修改文件: `ActionNode` 和 `FlowNode` 的各自基类 (或 `ENSNode.cs`)**
    ...
    *   【注】实际落地签名为 `public abstract void Execute(BaseNodeData data, EventNodeContext ctx, Action onComplete);`，以保留原有上下文与异步回调。
*   **修改文件: 非对话类 Action 节点**
...
*   **停止维护: 对话类节点**
    ...
    *   【实际处理补充】非 Yarn 对话节点已直接删除；Yarn 对话节点迁移为普通 Action 节点并完成 Data 拆分。
...
*   **删除文件: `SequenceFlowNode.cs`、`ChoiceDialogueNode.cs` 等旧分支控制节点**
    ...
    *   【实际处理补充】上述 Flow 节点已在 Phase 1 提前删除。
...
**当前实际进度补充**：
- Phase 1 已完成 `BaseNodeData` 与非对话 Action 的 Data 拆分。
- `EventNode.Execute` 已切换为 `BaseNodeData + EventNodeContext + Action` 签名。
- `EventRunnerService` 适配尚未开始。
