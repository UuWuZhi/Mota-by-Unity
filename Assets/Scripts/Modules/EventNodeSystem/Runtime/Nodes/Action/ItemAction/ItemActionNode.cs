...
namespace Modules.EventNodeSystem.Runtime.Nodes.Action.ItemAction
{
    ...
    public abstract class ItemActionNode : ActionNode
    {
        ...
        public sealed override void Execute(BaseNodeData data, EventNodeContext ctx, System.Action onComplete)
        {
            ...
            {
                ExecuteItem(data, itemCtx, onComplete);
            }
            else
            {
                DebugEditor.LogWarning(
                    $"{GetType().Name}: 需要 ItemEventContext，但收到 {ctx?.GetType().Name ?? "null"}，跳过执行。");
                onComplete?.Invoke();
            }
        }

        // 子类实现 Item 专用逻辑
        public abstract void ExecuteItem(BaseNodeData data, ItemEventContext ctx, System.Action onComplete);
    }
}