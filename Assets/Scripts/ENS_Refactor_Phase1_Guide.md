...
- **设计逻辑**：重构并替换原本的基类 `Execute` 签名。
  - *旧*：`public abstract void Execute(EventRunnerService runner);`
  - *新*：`public abstract void Execute(BaseNodeData data, EventRunnerService runner);`
  - *流程*：`EventRunnerService` 在读取到包含参数的 `BaseNodeData` 后，调用该节点享元 SO，并将这段数据“喂”给模板去触发表现。
