...
namespace Modules.EventNodeSystem.Runtime.Nodes.Condition
{
    ...
    public class PlayerHasItem : ConditionNode
    {
        ...
        public override void Evaluate(BaseNodeData data, EventNodeContext ctx, Action<bool> onResult)
        {
            ...
            {
                Debug.LogWarning($"{nameof(PlayerHasItem)}: 数据类型不匹配，期望 {nameof(PlayerHasItemData)}，默认返回 false。");
            ...
            try
            {
                ...
                {
                    Debug.LogError($"[{nameof(PlayerHasItem)}]: InventoryService 未配置，无法判断玩家物品数量。");
                    onResult?.Invoke(false);
                    return;
                }
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[PlayerHasItem]: 执行条件判断时发生异常，默认返回 false。{ex.Message}");
                result = false;
            }

            Debug.Log($"PlayerHasItem: 检测玩家物品 {conditionData.itemType} 实际数量：{(ctx?.GetService<IInventoryService>()?.GetItemCount(conditionData.itemType) ?? 0)}，比较：{conditionData.comparisonMode} {conditionData.requiredCount}，结果：{result}");
