...
namespace Modules.Player.Runtime.Movement
{
    ...
    {
        [Header("输入设置")] [SerializeField] private float inputInterval = 0.15f; // 输入间隔（也许有用呢？）

        [SerializeField] private bool useInputInterval; // 间隔开关（false=长按无间隔）
        ...
        private void Update()
        {
            if (IsInputBlocked) return; // 输入屏蔽时直接返回
            ...
            var newPressed = GetAbstractKeyDown();
            if (newPressed != AbstractKey.None) _lastPressedKey = newPressed;
            ...
            else
            {
                ...
                var held = GetAnyAbstractKeyHeld();
